...
class networks(object):
    ...
    def generator(self,nir,noise):
	input_ = tf.concat(3,[nir,noise])
	g_bn0 = batch_norm(self.batch_size,name='g_bn0')
        g_nir0 =lrelu(g_bn0(conv2d(input_,self.df_dim*2,name='g_nir0')))
	...
	tf.get_variable_scope().reuse_variables()
	g_bn0 = batch_norm(self.batch_size,name='g_bn0')
        g_nir0 =g_bn0(conv2d(nir,self.df_dim*2,name='g_nir0'),train=False)
        g_nir0_1 =lrelu(g_nir0)
	g_bn1 = batch_norm(self.batch_size,name='g_bn1')
        g_nir1 =g_bn1(conv2d(g_nir0_1,self.df_dim*4,name='g_nir1'),train=False)
        g_nir1_1 =lrelu(g_nir1)
	g_bn2 = batch_norm(self.batch_size,name='g_bn2')
        g_nir2 =g_bn2(conv2d(g_nir1_1,self.df_dim*4,name='g_nir2'),train=False)
        g_nir2_1 =lrelu(g_nir2)
	g_bn3 = batch_norm(self.batch_size,name='g_bn4')
        g_nir3 =g_bn3(conv2d(g_nir2_1,self.df_dim*2,name='g_nir3'),train=False)
        g_nir3_1 =lrelu(g_nir3)
        g_nir4 =conv2d(g_nir3_1,3,name='g_nir4')

	return tf.tanh(g_nir0), tf.tanh(g_nir1), tf.tanh(g_nir2), tf.tanh(g_nir3),tf.tanh(g_nir4)
