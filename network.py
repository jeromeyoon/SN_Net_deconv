...
class networks(object):
	...
	g_bn1 = batch_norm(self.batch_size,name='g_bn1')
        g_nir1 =tf.nn.relu(g_bn1(conv2d(g_nir0,self.df_dim*8,k_h=1,k_w=1,name='g_nir1')))
	...
	g_bn3 = batch_norm(self.batch_size,name='g_bn3')
        g_nir3 =tf.nn.relu(g_bn3(conv2d(g_nir2,self.df_dim*4,name='g_nir3')))
	g_bn4 = batch_norm(self.batch_size,name='g_bn4')
        g_nir4 =tf.nn.relu(g_bn4(conv2d(g_nir2,self.df_dim*2,name='g_nir4')))
        g_nir5 =conv2d(g_nir4,3,k_h=1,k_w=1,name='g_nir5')
	...
	d_bn3 = batch_norm(self.batch_size,name='d_bn3')
        h3 = conv2d(h2, self.df_dim*8, d_h=2,d_w=2,name='d_h3_conv') #output size: 4x4
        #h4 = conv2d(h3,1, k_h=1,k_w=1,name='d_h4_conv') #output size: 4x4
        h4 = lrelu(linear(tf.reshape(h3, [self.batch_size, -1]), 1024, 'd_h4_lin')) #output 512
	...
	tf.get_variable_scope().reuse_variables()
	g_bn0 = batch_norm(self.batch_size,name='g_bn0')
        g_nir0 =tf.nn.relu(g_bn0(conv2d(nir,self.df_dim*2,name='g_nir0'),train=False))
	g_bn1 = batch_norm(self.batch_size,name='g_bn1')
        g_nir1 =tf.nn.relu(g_bn1(conv2d(g_nir0,self.df_dim*8,k_h=1,k_w=1,name='g_nir1'),train=False))
	g_bn2 = batch_norm(self.batch_size,name='g_bn2')
        g_nir2 =tf.nn.relu(g_bn2(conv2d(g_nir1,self.df_dim*4,name='g_nir2'),train=False))
	g_bn3 = batch_norm(self.batch_size,name='g_bn3')
        g_nir3 =tf.nn.relu(g_bn3(conv2d(g_nir2,self.df_dim*4,name='g_nir3'),train=False))
	g_bn4 = batch_norm(self.batch_size,name='g_bn4')
        g_nir4 =tf.nn.relu(g_bn4(conv2d(g_nir2,self.df_dim*2,name='g_nir4'),train=False))
        g_nir5 =conv2d(g_nir4,3,k_h=1,k_w=1,name='g_nir5')
